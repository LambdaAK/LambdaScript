...
Consequently, the dyanmic semantics of the bind expression is already defined by the dynamic semantics of the function and function application
...
Now, evaluate the expression between $\leftarrow$ and "in", to a function closure subject to the following modification: the environment of the closure contains a binding to itself
$$(\Delta_r, x_1, \text{ fn } x_2 \rightarrow \dots \text{ fn } x_n \rightarrow e_1)$$
where
$$\Delta_r(f) = (\Delta_r, x_1, \text{ fn } x_2 \rightarrow \dots \text{ fn } x_n \rightarrow e_1)$$

The remainder of the dynamic semantics for the recursive bind expression is the same as for the standard bind expression
