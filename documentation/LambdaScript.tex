...
\title{Forge Syntax and Semantics}
\author{Alex Kozik}
\begin{document}
\large
...
Forge uses an environment model to make substitutions in function bodies. The environment is an object defined as follows
...
A function in Forge evaluates to a function closure, which consists of three parts: the environment, the pattern, and the body.
...
In order to discuss the relationships between the expressions of Forge and the corresponding types, we must first define a type relation.
